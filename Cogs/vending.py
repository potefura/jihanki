...
class VendingMachineCog(commands.Cog):
    ...
    async def vm_create(self, interaction: discord.Interaction, name: str):
        ...
        msg_parts = [f"自販機「{name}」を作成しました。\n**自販機ID:** `{new_vm_id}`"]
        
        if not paypay_id:
            msg_parts.append("PayPayアカウントが未登録です。`/paypay登録` を実行してください。")
        if not kyash_id:
    ...
    class CouponModal(ui.Modal, title="購入情報入力"):
        ...
        async def on_submit(self, interaction: discord.Interaction):
            ...
            embed.add_field(
                name="決済方法",
                value=f"```{self.payment_method.upper()}```",
                inline=False
            )
            ...
            try:
                ...
                if self.final_price > 0 and not skip_payment:
                    if self.payment_method == "paypay":
                        ...
                        # ===== セッション保持でリンク確認から決済まで実行 =====
                        async with paypayu.PayPayAsync(
                            phone=owner_credentials["phone"],
                            password=owner_credentials["password"],
                            uuid=owner_credentials["uuid"]
                        ) as pp:
                            # 初回ログイン（セッション確立）
                            ...
                            # ログイン済みセッションでリンク情報確認
                            payment_info = await pp.check_link(link)
                            if not payment_info:
                                return await interaction.followup.send(
                                    "有効なPayPayリンクを入力してください。",
                                    ephemeral=True
                                )
                            ...
                            # ===== 受取保留対策（同一セッション） =====
                            if result != True and initial_status == "PENDING":
                                try:
                                    import asyncio
                                    ...
                                    # ログイン済みセッションで3分後のリンク状態確認
                                    payment_info = await pp.check_link(link)
                                    if payment_info:
                                        new_status = payment_info.get("payload", {}).get("orderStatus")
                                        print(f"初回状態: {initial_status}")
                                        print(f"1分後の状態: {new_status}")
                                        if initial_status == "PENDING" and new_status in ["SUCCESS", "COMPLETED"]:
                                            result = True
                                except Exception as e:
                                    print(f"保留確認エラー: {e}")
                ...
                try:
                    ...
                    dm_embed.set_footer(text="Developer @potefura")
                ...
                if vm.get("private_log_channel_id"):
                    try:
                        ...
                        if private_log_channel:
                            ...
                            private_log_embed.set_footer(text="Developer @potefura")
    ...
    class PayPayModal(ui.Modal, title="PayPay決済"):
        ...
        async def on_submit(self, interaction: discord.Interaction):
            link = self.paypay_input.value.strip()

            # ① 重複チェック (is_link_used がある前提)
            if is_link_used(link):
                return await interaction.response.send_message(
                    "❌ そのPayPayリンクは既に使用されています。", 
                    ephemeral=True
                )
            ...
            # ③ 決済処理へ進む
            # ここでは interaction.response.defer() 等を絶対に呼び出さないこと。
            # process_purchase メソッド内で defer が呼ばれるため、
            # 呼び出し元(on_submit)で応答を返すと「二重応答」エラーになります。
