...
class LTCCheckoutSourceTests(unittest.TestCase):
    ...
    def test_legacy_prices_are_migrated_before_products_are_listed(self):
        source = VENDING_COG.read_text(encoding="utf-8")

        self.assertIn("def migrate_vending_prices(", source)
        self.assertIn("migrate_vending_prices(data)", source)
        self.assertIn('return int(product.get("price", 0))', source)
